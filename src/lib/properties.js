...
export async function searchProperties(filters = {}) {
    ...
    // 4. Price Range
    if (filters.minPrice) {
        queries.push(Query.greaterThanEqual('price', parseInt(filters.minPrice, 10)));
    }
    if (filters.maxPrice) {
        queries.push(Query.lessThanEqual('price', parseInt(filters.maxPrice, 10)));
    }

    // 5. Attributes (Bads/Baths)
    if (filters.beds && filters.beds !== 'any') {
        queries.push(Query.greaterThanEqual('beds', parseInt(filters.beds, 10)));
    }
...
export async function createProperty(data) {
    ...
    try {
        ...
        const payload = {
            ...
            // Extended Attributes
            beds: parseInt(data.beds, 10) || 0,
            baths: parseInt(data.baths, 10) || 0,
            size_sqft: parseInt(data.size_sqft, 10) || 0,
