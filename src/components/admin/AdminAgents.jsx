                    ...
                    <tr key={agent.$id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center overflow-hidden relative">
                            {agent.avatar ? (
                              <Image
                                src={agent.avatar.startsWith('http') ? agent.avatar : `${process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT || 'https://sgp.cloud.appwrite.io/v1'}/storage/buckets/listing_images/files/${agent.avatar}/view?project=${process.env.NEXT_PUBLIC_APPWRITE_PROJECT_ID || 'landsalelkproject'}`}
                                alt={agent.name}
                                fill
