...
export function ListingAnalytics({ listing, onClose }) {
    // Generate stable mock data using useMemo to prevent re-render changes
    const data = useMemo(() => {
        const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
        // Use listing ID as seed for consistent "random" values
        const seed = listing?.$id ? listing.$id.charCodeAt(0) : 1;
        return days.map((day, i) => ({
            name: day,
            views: Math.floor((seed * (i + 1) * 7) % 50) + 10,
            favorites: Math.floor((seed * (i + 1)) % 5),
            clicks: Math.floor((seed * (i + 1) * 3) % 20) + 5
        }));
    }, [listing?.$id]);
            ...
            <div className="bg-white rounded-2xl w-full max-w-3xl p-6 shadow-2xl animate-scale-up max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-start mb-6">
                    <div>
                        ...
                        <p className="text-amber-600 text-xs mt-2 bg-amber-50 px-2 py-1 rounded-md inline-block">⚠️ Preview Data - Real analytics coming soon</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                        ...
                        <div className="text-xs text-slate-400 mt-1">— preview data</div>
                    </div>
