...
export default function CreateListingPage() {
  ...
  const [legalDocs, setLegalDocs] = useState([]); // [MODIFY] New state for legal docs
  ...
  const handleLegalDocsChange = (e) => {
    const files = Array.from(e.target.files);
    setLegalDocs((prev) => [...prev, ...files]);
  };

  const removeLegalDoc = (idx) => {
    ...
    try {
      ...
      for (const doc of legalDocs) {
        try {
          ...
          const url = `${endpoint}/storage/buckets/${bucketId}/files/${res.$id}/view?project=${projectId}`;
          legalDocIds.push(url);
        } catch (err) {
          console.error("Legal doc upload failed:", err);
          // Optional: count failures or warn user
        }
      }
  ...
  // Don't render until mounted AND auth checked AND authenticated
  if (!mounted || !authChecked || !isAuthenticated) return null;
        ...
        <div className="mb-8 flex items-center justify-center gap-2 md:mb-12">
          {[1, 2, 3, 4].map((s) => (
            <div key={s} className="flex items-center">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold transition-all ${step === s
                ...
                <div className="mb-8 grid grid-cols-3 gap-3 md:gap-4">
                  {propertyTypes.map((type) => (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center rounded-2xl border-2 p-4 text-center transition-all md:p-6 ${formData.category_id === type.id
                ...
                <div className="mb-8 grid grid-cols-2 gap-4">
                  {listingTypes.map((type) => (
                    <button
                      ...
                      className={`rounded-2xl border-2 p-4 text-left transition-all ${formData.listing_type === type.id
              ...
              <div className="animate-fade-in">
                ...
                <div className="mt-8 border-t border-slate-200 pt-8">
                  ...
                  <p className="mb-4 text-sm text-slate-500">
                    Upload photos of deeds, survey plans, or other documents to build trust.
                    Changes here will be visible on the listing page.
                  </p>
                    ...
                    <div className="mb-4 grid grid-cols-2 gap-4 md:grid-cols-4">
                      {legalDocs.map((file, idx) => (
                        <div key={idx} className="relative aspect-[3/4] overflow-hidden rounded-xl border border-slate-200">
                          ...
                          <button
                            type="button"
                            onClick={() => removeLegalDoc(idx)}
                            className="absolute right-2 top-2 rounded-full bg-white/80 p-1 text-red-500 backdrop-blur-sm transition-colors hover:bg-white"
                          >
                            <X className="h-4 w-4" />
                          </button>
                        </div>
                  ...
                  <label className="flex w-full cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 py-8 transition-colors hover:bg-slate-100">
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full rounded-xl border bg-white py-3 pr-4 pl-10 transition-colors outline-none ${phoneErrors.contact_phone
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full rounded-xl border bg-white p-3 transition-colors outline-none ${phoneErrors.owner_phone
                          ? "border-red-300 focus:border-red-500"
            ...
            {step < 4 && (
              <div className="mt-8 flex justify-between border-t border-slate-100 pt-6">
                <button
                  ...
                  onClick={prevStep}
                  disabled={step === 1}
                  className={`flex items-center gap-2 rounded-xl px-6 py-3 font-bold transition-all ${step === 1
