...
export async function getAgentCertificate(agentData) {
    try {
        ...
        console.warn('Server-side certificate generation failed', error);
        // Client-side fallback should be handled in the component that uses this
        // Don't import client-side libraries here to avoid SSR/build issues
        return { success: false, error: error.message || 'Server-side generation failed. Please try again.', fallback: true };
    }
}
