// Import remaining listings (skip first 100 already imported)
...
console.log("=== IMPORT REMAINING LISTINGS ===\n");
...
async function main() {
    const tempDir = path.resolve('temp_remaining');
    ...
    // Skip first 100 already imported
    const toImport = data.slice(100);
    console.log(`Total: ${data.length}, Skipping first 100, Importing: ${toImport.length}\n`);

    let success = 0;
    let failed = 0;

    for (const [i, item] of toImport.entries()) {
        process.stdout.write(`[${i + 1}/${toImport.length}] `);

        // Upload 1 image per listing (faster)
        const imageIds: string[] = [];
        const imgUrl = (item.images_source || [])[0];
        if (imgUrl) {
            try {
    ...
    console.log(`\n=== DONE: ${success}/${toImport.length} imported ===`);
