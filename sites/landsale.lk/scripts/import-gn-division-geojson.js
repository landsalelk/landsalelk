...
function processGeoJSONFeature(feature) {
  ...
  const sampleRealEstateData = {
    avg_land_price: Math.round((areaKm2 * 1000 + Math.random() * 5000) * 100) / 100, // Random price per sqm
    avg_house_price: Math.round((areaKm2 * 2000 + Math.random() * 10000) * 100) / 100,
  ...
  const document = {
    ...
    // Real estate data (sample - would be populated from actual sources)
    ...
    elevation_avg: Math.round((Math.random() * 100 + 10) * 10) / 10,
...
async function importGNDivisionData() {
  console.log('🚀 Starting GN Division Data Import from GeoJSON')
  console.log('====================================================')
  ...
  try {
    ...
    for (let i = 0; i < features.length; i++) {
      ...
      try {
        ...
        console.log(`\n📍 Processing ${i + 1}/${features.length}: ${documentData.name_en} (${documentData.gn_code})`)
      ...
      if ((i + 1) % 10 === 0) {
        console.log(`\n📈 Progress: ${i + 1}/${features.length} processed (${successCount} successful, ${errorCount} errors)`)
      }
    }
...
// Main function
async function main() {
  console.log('🚀 Sri Lanka GN Division Data Import - Method 4 (HDX GeoJSON)')
  console.log('============================================================')
  
  try {
    // Import data
    await importGNDivisionData()
    ...
    console.log('   2. Update with real demographic data from census')
    console.log('   3. Add real estate market data')
