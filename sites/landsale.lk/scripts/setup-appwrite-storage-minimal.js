...
/**
 * Appwrite Storage Setup Script - Minimal Version
...
async function setupAppwriteStorage() {
    ...
    try {
        ...
        for (const config of BUCKET_CONFIGS) {
            try {
                if (existingBucketIds.includes(config.bucketId)) {
                    ...
                    // Try to update with minimal parameters
                    try {
                        ...
                        console.log(`⚠️  Could not update bucket ${config.name}, but it already exists`)
                    }
                } else {
                    console.log(`➕ Creating new bucket: ${config.name} (${config.bucketId})`)
                    
                    // Create bucket with minimal parameters first
        ...
        console.log('   2. Set file extensions and size limits')
        console.log('   3. Test file uploads in your application')
        console.log('   4. Verify bucket permissions work correctly')
