/**
 * Import GN Division Data from GeoJSON - Final Corrected Version
 * This script processes the administrative boundary data and imports it into Appwrite
 * Handles nested objects as JSON strings due to Appwrite limitations
 */
...
function processGeoJSONFeature(feature) {
  ...
  // Create Appwrite document with JSON strings for nested objects
  const document = {
    ...
    active: true,
    coordinates: JSON.stringify(coordinates), // Convert to JSON string
    demographics: JSON.stringify(sampleDemographics), // Convert to JSON string
    land_parcel_count: Math.floor(areaKm2 * 20 + Math.random() * 100),
    land_registry: JSON.stringify(sampleLandRegistry), // Convert to JSON string
    real_estate_data: JSON.stringify(sampleRealEstateData), // Convert to JSON string
...
async function importGNDivisionData() {
  console.log('🚀 Starting GN Division Data Import from GeoJSON (Final Corrected)')
  console.log('====================================================================')
  ...
  try {
    ...
    // Process first 50 for testing (you can increase this number)
    const testLimit = 50
    ...
    if (documents.length > 0) {
      ...
      documents.slice(0, 5).forEach((doc, index) => {
        ...
        try {
          ...
          demographics = JSON.parse(doc.demographics)
          realEstateData = JSON.parse(doc.real_estate_data)
        } catch (parseError) {
        ...
        if (realEstateData) {
          console.log(`      🏠 Properties: ${realEstateData.property_count}`)
        }
      })
      
      // Show data quality metrics
      const withCoordinates = documents.filter(doc => doc.coordinates).length
      const withArea = documents.filter(doc => doc.area_km2 > 0).length
      const withDemographics = documents.filter(doc => doc.demographics).length
      
      console.log('\n📈 Data Quality Metrics:')
      console.log(`   📍 With coordinates: ${withCoordinates}/${documents.length}`)
      console.log(`   📏 With area data: ${withArea}/${documents.length}`)
      console.log(`   👥 With demographics: ${withDemographics}/${documents.length}`)
    }
...
// Main function
async function main() {
  console.log('🚀 Sri Lanka GN Division Data Import - Method 4 (HDX GeoJSON) - FINAL')
  console.log('=====================================================================')
  ...
  try {
    ...
    console.log('   ✅ Nested objects handled as JSON strings')
