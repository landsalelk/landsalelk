...
export async function POST(req: NextRequest) {
    try {
        ...
        const { 
            ...
            customAmount, 
            customName,
            useOnsiteCheckout = false // New parameter for onsite checkout
        } = body
        ...
        // If onsite checkout is requested, return the payment data for SDK use
        if (useOnsiteCheckout) {
            return NextResponse.json({
                success: true,
                useOnsiteCheckout: true,
                paymentData,
                product,
                environment: PAYHERE_MODE,
                embedUrl: PAYHERE_URL
            })
        }

        // Otherwise, return traditional redirect flow
        return NextResponse.json({
            success: true,
