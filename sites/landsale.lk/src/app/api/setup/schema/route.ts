...
export async function POST() {
    ...
    try {
        log('🚀 Starting Appwrite Schema Setup...')
        log(`   Endpoint: ${APPWRITE_ENDPOINT}`)
        log(`   Project: ${APPWRITE_PROJECT_ID}`)
        log(`   Database: ${DATABASE_ID}`)

        if (!APPWRITE_API_KEY) {
            return NextResponse.json({ success: false, error: 'APPWRITE_API_KEY is required', logs })
        }
        ...
        try {
            ...
            log('   ✅ Created agents collection')
        } catch (e: any) {
            if (e.code === 409) {
                log('   ⏭️  agents collection already exists')
            } else {
                log(`   ❌ Failed to create agents: ${e.message}`)
            }
        }
        ...
        for (const attr of agentAttributes) {
            try {
                ...
                log(`   ✅ Created attribute: ${attr.key}`)
            } catch (e: any) {
                if (e.code === 409) {
                    log(`   ⏭️  Attribute ${attr.key} already exists`)
                } else {
                    log(`   ❌ Failed to create ${attr.key}: ${e.message}`)
                }
            }
        }
        ...
        try {
            ...
            log('   ✅ Created agent_leads collection')
        } catch (e: any) {
            if (e.code === 409) {
                log('   ⏭️  agent_leads collection already exists')
            } else {
                log(`   ❌ Failed to create agent_leads: ${e.message}`)
            }
        }
        ...
        // ==========================================
        // 3. AGENT_DOCUMENTS BUCKET
        // ==========================================
        log('\n📦 Setting up AGENT_DOCUMENTS bucket...')
        ...
        try {
            ...
            log('   ✅ Created agent_documents bucket')
        } catch (e: any) {
            if (e.code === 409) {
                log('   ⏭️  agent_documents bucket already exists')
            } else {
                log(`   ❌ Failed to create bucket: ${e.message}`)
            }
        }

        // ==========================================
        // 4. CREATE INDEXES (wait a bit first)
        // ==========================================
        ...
        for (const idx of indexes) {
            try {
                ...
                log(`   ✅ Created index: ${idx.collection}.${idx.key}`)
            } catch (e: any) {
                if (e.code === 409) {
                    log(`   ⏭️  Index ${idx.key} already exists`)
                } else {
                    log(`   ❌ Failed to create index ${idx.key}: ${e.message}`)
                }
            }
        }
