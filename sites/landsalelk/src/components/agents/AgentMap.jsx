...
export default function AgentMap({ agents }) {
    ...
    return (
        ...
        >
            ...
            {agents.map(agent => {
                // Mock coords if missing (Spread random agents around Colombo for demo)
                const lat = agent.lat || (6.9 + (Math.random() * 0.1));
                const lng = agent.lng || (79.8 + (Math.random() * 0.1));

                return (
                    <Marker key={agent.$id} position={[lat, lng]} icon={customIcon}>
